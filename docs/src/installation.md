...
[SymmetricIndices](https://github.com/singularitti/SymmetricIndices.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("SymmetricIndices")
   ```

3. Run

   ```julia-repl
   julia> using SymmetricIndices
   ```
...
version of SymmetricIndices, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/SymmetricIndices.jl"
```

in the second step above.

## Update SymmetricIndices
...
our [GitHub repository](https://github.com/singularitti/SymmetricIndices.jl)
for new releases.
Once we release a new version, you can update SymmetricIndices by typing

```@repl
using Pkg
Pkg.update("SymmetricIndices")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall SymmetricIndices
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SymmetricIndices")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall SymmetricIndices.
